...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are SORA, a sharp and efficient AI assistant. "
...
_MEMORY_EVERY_N_TURNS = 1 # Update memory every turn (highest retention)
_last_memory_input    = ""


def _update_memory_async(user_text: str, sora_text: str) -> None:
    """
...
class SoraLive:
    ...
    def __init__(self, ui: SoraUI):
        self.ui             = ui
        self.session        = None
        self.audio_in_queue = None
        self.out_queue      = None
        self._loop          = None
        if hasattr(self.ui, "set_chat_callback"):
            self.ui.set_chat_callback(self.speak)
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[SORA] 🔧 TOOL: {name}  ARGS: {args}")
    ...
    async def _listen_audio(self):
        ...
        try:
            ...
            print(f"[SORA] ❌ Mic error: {e}")
            raise
        finally:
            stream.close()

    async def _receive_audio(self):
        print("[SORA] 👂 Recv started")
        out_buf = []
        in_buf  = []
        ...
        try:
                ...
                async for response in turn:
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if out_buf:
                                ...
                                if full_out:
                                    self.ui.write_log(f"Sora: {full_out}")
                            out_buf = []
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            print(f"[SORA] 📞 Tool call: {fc.name}")
            ...
            print(f"[SORA] ❌ Recv error: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[SORA] 🔊 Play started")
        stream = await asyncio.to_thread(
            pya.open,
            format=FORMAT,
            channels=CHANNELS,
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[SORA] ✅ Connected.")
                    self.ui.write_log("SORA online.")
                ...
                print(f"[SORA] ⚠️  Error: {e}")
                traceback.print_exc()

            print("[SORA] 🔄 Reconnecting in 3s...")
